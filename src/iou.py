...
class IoU(keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        union = K.sum(K.cast((y_true + y_pred) > 0, 'float32'))

        self.intersection.assign_add(intersection)
        self.union.assign_add(union)

    def result(self):
        iou = (self.intersection + K.epsilon()) / (self.union + K.epsilon())
        return iou
...
es = EarlyStopping(monitor='val_loss', mode='max', verbose=1, patience=EARLY_STOP_PATIENCE)
...
# model.load_weights('../resource/dataset/train_output/model_unet_iou_final_weights.h5')
